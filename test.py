...
# most important aspects of a filter is setting the esariances correctly.timated sensor v
...
################################################################################################
# We can also set up some constants that  iteration of won't change for any our solver.
################################################################################################
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    #     if gnss.t[i] == imu_f.t[k-1]:
    #         p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss.data[i].T, p_est[k], v_est[k], q_est[k])
    #  激光雷达与gps的更新频率可能不一样，要分别利用二者进行更新
    # 因为观测数据过少，更新效果比较差
